...
class ChangeMailNewCodeValidator extends ConstraintValidator
{
    ...
    public function validate($value, Constraint $constraint): void
    {
        ...
        if (!$emailRequest ||
            !$emailRequest->getNewEmailCode() ||
            $value !== $emailRequest->getNewEmailCode()->getCode()
        ) {
            $this->context->buildViolation($constraint->message)
                ->setParameter('{{message}}', $this->translator->trans(
                    'change_email.invalid_code.new_email',
                ))
                ->addViolation();
        }
    }
