...
class BalanceViewFactoryTest extends TestCase
{
    ...
    {
        $tokens = [
            ['name' => 'foo', 'hidden' => true, 'crypto' => true, 'lockIn' => true, 'deployed' => true],
            ['name' => 'bar', 'hidden' => false, 'crypto' => true, 'lockIn' => false, 'deployed' => true],
            ['name' => 'baz', 'hidden' => false, 'crypto' => false, 'lockIn' => true, 'deployed' => true],
            ['name' => 'qux', 'hidden' => true, 'crypto' => false, 'lockIn' => false, 'deployed' => true],
            ['name' => 'empty', 'hidden' => null, 'crypto' => false, 'lockIn' => false, 'deployed' => false],
            ['name' => 'lok', 'hidden' => false, 'crypto' => true, 'lockIn' => true, 'deployed' => false],
        ];
        ...
        $this->assertEquals([
            'foo' => ['1', '1', '1', 'FOO', 'fooBAR', 4, false, true, true],
            'bar' => ['1', '1', null, 'FOO', 'barBAR', 4, true, true, true],
            'baz' => ['1', null, '1', 'baz', 'bazBAR', 4, false, false, true],
            'qux' => ['1', null, null, 'qux', 'quxBAR', 4, false, false, true],
            'lok' => ['1', '1', '1', 'FOO', 'lokBAR', 4, true, true, false],
        ...
        $tm->method('findByName')->willReturnCallback(function ($name) use ($tokens): ?Token {
            foreach ($tokens as $token) {
                if (false === $token['hidden'] && $token['name'] === $name) {
                    return $this->mockToken($name, $token['crypto'], $token['lockIn'], $token['hidden'], $token['deployed']);
                }
            }
    ...
    private function mockToken(string $name, bool $hasCrypto, bool $hasLockIn, bool $isHidden, bool $deployed): Token
    {
